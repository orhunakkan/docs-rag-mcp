...
Counts above are from the last sync in this environment and will drift as upstream docs grow — see [Keeping the docs up to date](#keeping-the-docs-up-to-date). They count *indexed chunks*, which since the embedding-window cap is more than the number of heading sections: this corpus's 4,940 sections become 6,308 chunks (`npm run probe` reports both). The Node.js runtime count is the exception: since that corpus is pinned to a fixed git tag rather than a moving branch, it only changes if `NODE_TAG` in `src/node/sources.ts` is bumped to a different release.
...
Then build the search indexes (first run downloads the ~90 MB embedding model and caches it in `.cache/`; the first *query* additionally downloads the ~23 MB reranking model there):
...
If you only want to rebuild the *index* — after a chunker or embedding change, say — use `npm run reindex` instead. It rebuilds from the normalized markdown already in `docs/` and skips the clone entirely, which is both much faster and the only way to measure a pipeline change honestly: a sync re-clones, so it can change the corpus at the same time as the code and leave you unable to attribute what moved.
...
| `npm run inspect` | Prints the top-N results per benchmark query with the label's match marked, so a wrong label can be told apart from a wrong result; `-- <corpus>` narrows it, `-- --q pw '<query>'` runs an ad-hoc one |
| `npm run probe` | Measures chunk sizes against the embedding window over `docs/`, and what splitting does to them — no clone, no embedding, no index. `-- --max 460` tries a different budget |
| `npm run reindex` | Rebuilds the indexes from the committed `docs/` without cloning upstream. `-- --no-split` / `-- --no-prefix` turn off one pipeline change at a time |
...
- **Every chunk is capped at the embedding model's input window, because overflow was being discarded in silence.** `Xenova/all-MiniLM-L6-v2` has `model_max_length: 512` and truncates anything longer before embedding — no error, no warning. Heading-sliced chunks run from 8 to 52,654 characters, so this was routine rather than exceptional: measured with the model's own tokenizer, **43% of the Playwright corpus's tokens and 14-26% of the other three never reached the embedder.** BM25 still indexed that text, so it stayed keyword-findable while the vector half of hybrid search could not see it at all — which is exactly why natural-language queries, the ones that depend on the vector side, were the weak group. The demonstration that convinced: two texts sharing a 3,480-character prefix and having entirely different tails embed to cosine similarity `1.000000`, bit-identical vectors. `src/chunk/split.ts` now splits oversized chunks at structural boundaries, and `npm run probe` asserts the result over the whole committed `docs/` tree without embedding anything.
- **The chunk budget is measured with the real tokenizer, not a characters-per-token constant.** Prose in these corpora runs about 4.3 characters per token and markdown with code fences about 2.0 — and the chunks that overflow worst are precisely the code-heavy API-reference ones, so any constant would be wrong where it matters most. Tokenizing every chunk once per sync is negligible beside the embedding pass. This does couple the chunker to the embedding model, which is a real cost: `MODEL_MAX_TOKENS` sits next to `MODEL_NAME` and `EMBEDDING_DIM` in `src/search/embed.ts` so that a model swap is known to touch all three. The cap is 440 tokens, leaving 72 under the window for the heading prefix below (measured at a 51-token maximum across all four corpora) plus the extractor's two special tokens. Splitting costs chunk count — Playwright grows 74%, the corpora together 35% — and the cap is set as high as that headroom allows precisely because a whole method's documentation staying in one chunk retrieves better than the same text in two.
- **Splitting respects structure, and a list item outranks a paragraph.** Never inside a fenced code block; a fence longer than the budget on its own is cut at line boundaries and closed and reopened with the same info string, so every piece is still renderable markdown. Never mid-line, which is what protects table rows and lines of code — the one thing that can exceed the budget is a single line that already does, and the probe counts them (11, all in MDN). The non-obvious part is list items: Playwright and Node write an option as a marker line naming it plus a separate indented paragraph explaining it, so splitting on the paragraph produced chunks that described an option without ever naming it. Items are kept whole, including nested ones, and only broken up when a single item exceeds the budget.
- **The heading path is embedded but never stored.** `indexChunks` used to embed `content` alone, so the vector had no idea a chunk sits under `class-page` → `Methods` → `screenshot`: BM25 saw `title` with a 3× boost and the vector side saw nothing. It now embeds `"${title} > ${headingPath}: ${content}"` while storing `content` byte-unchanged — the stored value is what the agent reads and what `dedupeByContent` compares, and a synthetic prefix in it would corrupt both. This matters most for deep API-reference sections, whose body is a parameter list whose prose never names the class it belongs to.
- **A cross-encoder reranks the candidates, because the ranking was wrong far more often than the retrieval was.** Before it, recall@1 was 64% while recall@5 was 89% — almost every miss had the right answer sitting at rank 2–5, which is the signature of a ranking problem rather than a retrieval one. `src/search/rerank.ts` scores the query and each candidate *together* with `Xenova/ms-marco-MiniLM-L-6-v2` (~23 MB, cached alongside the embedder), which is why it can beat cosine similarity over vectors computed independently and long before the query existed. It is far too slow to run over a corpus and exactly right for 25 candidates: hybrid retrieve → collapse duplicates → rerank → collapse sibling parts → slice to `limit`. Measured, that is **recall@1 64% → 70%, recall@3 79% → 89%, MRR 0.723 → 0.789**, and it costs median query latency **13 ms → 440 ms** — the reason it is a `Tuning` switch rather than a hardcoded step. One corpus does not benefit: TypeScript's recall@3 drops 75% → 71%, since an ms-marco-trained model sometimes prefers a handbook index page to the section that answers the question.
- **Sibling parts of one section are collapsed, and *after* reranking rather than before.** Capping chunk size means one heading section can be several overlapping chunks, so a query matching the section tends to match several of its parts — on "difference between an interface and a type alias", five of six result slots were parts of just two sections. Since sibling parts share a `sourceUrl`, a `title` and a `headingPath`, an agent receiving two of them cannot tell them apart, so all but the best-matching one are dropped with no `alsoAt` annotation (unlike the cross-class duplicates below, where the folded-in URLs are real information). Where this runs was measured, not assumed: collapsing before reranking narrows the cross-encoder's candidate pool to one chunk per section and cost a point of recall@1 and three of recall@3, so it runs last.
- **Hybrid search tuning matters more than it looks.** Orama's default vector-similarity cutoff (0.8) is tuned for larger embedding models and silently drops nearly every result for this MiniLM model's short-text embeddings — degenerating "hybrid" search into plain keyword search. `src/search/tuning.ts` lowers this to `0.1` and splits the blend evenly between BM25 and vector. If you swap the embedding model, re-validate with `npm run benchmark`.
- **The tuning is benchmarked, and it was re-swept after the retrieval changes — which overturned one of its own findings.** `npm run benchmark -- --sweep` grids `hybridWeights` × `similarity` × `titleBoost` × rerank candidate count over the labelled queries. Findings worth not rediscovering: the committed `0.5/0.5` is the best row by overall MRR (0.799) *and* beats the previous `0.3/0.7` on verbose-identifier recall@1 (58% vs 54%) at no cost to natural language; `similarity` is inert anywhere in 0.05–0.2, so the cutoff is not the binding constraint at those levels; `titleBoost: 3` clearly beats `1`, while `6` trades 4 points of verbose recall for 3 of natural-language recall and so remains a wash; and `rerankCandidates: 25` beats both 10 and 40 — a wider pool is *not* better, because it gives the cross-encoder more chances to promote something wrong. The earlier sweep concluded that "shifting weight toward text degrades both monotonically", and that is no longer true: with the whole chunk actually reaching the embedder, verbose-identifier recall now peaks at `text 0.7` (65%), though only by dropping natural-language recall to 45%. The weights were re-swept precisely because steps that change what the retriever *sees* invalidate a tuning measured before them — if you change the chunking or the embedding model, re-sweep.
- **A benchmark label is as easy to get wrong as the retrieval it scores, in both directions.** Labels are URL substrings, so a label that is too strict rejects a correct answer living at a different URL, and one that is too loose scores any chunk from a whole page or module as correct. Both were present, and the loose direction was the larger error: labelling Node's queries by module (`fs.html`) meant `fs.readFile` scored rank 1 on `fs.html#fsfsyncsyncfd`, and Node's recall@1 reads 92% under module labels versus 50% under anchor-precise ones — the same index, measured twice. Auditing every top-5 by hand moved overall recall@1 from a reported 78% to a real 60%. `tests/fixtures/benchmark-queries.ts` now states the labelling rules it follows, records inline why each non-obvious label member qualifies (and why plausible-looking near-misses were excluded), lets a label end in `$` to anchor to the end of a URL — `Array/reduce` is otherwise a substring of `Array/reduceRight`, which was silently scoring as a hit — and tags every query with an explicit `kind` rather than letting the report infer grouping from the query's `id`. `npm run inspect` prints the top-N with the label's match marked, which is what tells a wrong label apart from a wrong result.
- **Content-free headings are never indexed.** `src/chunk/emptySection.ts` drops any chunk that, once its own heading line is removed, has nothing but whitespace left. This was originally inline in the MDN chunker for macro-stripped sections; measuring the corpora showed the same defect untreated in Playwright (539 chunks, 7.8%) and TypeScript (49, 3.2%) — Playwright's API pages put a bare `## Methods` / `## Properties` / `## Events` above their first `###`, so `"## Methods"` was being embedded and indexed 182 times as a retrievable result. The rule is deliberately structural rather than a `length < N` threshold: heading-section lengths run from 8 to 19,755 characters with no gap to justify a cutoff, and a threshold would discard terse-but-real sections. (This rule runs on heading sections, before the embedding-window cap splits the long ones — an empty section has nothing to split.) A section whose only body is a fenced code block is kept, which is the case a naive "looks empty" check gets wrong.
- **Duplicate content is collapsed at query time, not at index time.** The four copies of `getByText` differ in `sourceUrl`, and which class you reached a method from is real information — so all four stay indexed, and `src/search/dedupe.ts` collapses them per query, keeping the highest-scoring copy and preserving the rest as `alsoAt`. One result saying "also on Frame, Locator, Page" beats both four identical results and one result that silently hides the other three. Because `limit` is passed straight to Orama, deduping afterwards would under-deliver, so queries over-fetch 4× what they need (capped at 100) and slice after collapsing — with reranking on, "what they need" is `rerankCandidates`, not `limit`, since a reranker can only reorder what it is handed. Around 16% of the Playwright corpus and 10% of the MDN corpus was redundant this way when measured, before the embedding-window cap changed the chunk boundaries; the mechanism is unaffected either way.
...
- **Natural-language queries are still the weak spot, though much less so.** On the 102-query benchmark (`npm run benchmark`) bare API identifiers score 88% recall@1 pooled across corpora, while questions phrased without an identifier score 58% — up from 42% before the chunk cap and the reranker, but still the weakest group. The tuning sweep shows this is not fixable by rebalancing the hybrid weights; see the design notes above and [`RETRIEVAL-PLAN.md`](RETRIEVAL-PLAN.md) for what moved it and by how much.
- **TypeScript is the weakest corpus, at 58% recall@1 against JavaScript's 87%,** and the only one where the cross-encoder makes things worse (its recall@3 drops 75% → 71%). Three natural-language questions miss the top 5 entirely — "make the compiler check my code more strictly", "should I use an enum or a plain object of constants", "require a type parameter to have certain properties" — in every case with the correct section present in the index and retrievable, so these are ranking failures rather than coverage gaps.
- **Some TypeScript handbook sections are not indexed at all.** `Narrowing.md` puts `# Discriminated unions`, `# The never type` and `# Exhaustiveness checking` as level-1 headings in the middle of the document, and the chunkers treat level 1 as the document title — so those sections never become chunks of their own and their text is absorbed into whichever level-2 section precedes them. Searching for "discriminated unions" finds an equivalent explanation on another page, but not the handbook's own.
- **Verbose identifier queries land the right answer in the top 3 (92%) far more reliably than at rank 1 (54%).** Adding true, relevant prose around an exact API name still costs precision at the top; nothing in the retrieval work so far moved that number.
...
**The first tool call after starting the server is slow.** Expected. The server loads its index lazily on the first call, and the first call also downloads (once) and initialises two models — the embedder and the reranker. Steady-state query latency is around 440 ms, most of it the reranker's ~25 forward passes; set `rerank: false` in `src/search/tuning.ts` to get back to ~13 ms at the cost of 6 points of recall@1 and 10 of recall@3.
