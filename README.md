...
- **The tuning is benchmarked, and rebalancing toward keyword search was tested and rejected.** `npm run benchmark -- --sweep` grids `hybridWeights` × `similarity` × `titleBoost` over the labelled queries. Findings worth not rediscovering: the committed `0.3/0.7` ties for the best overall MRR *and* the best recall on verbose identifier queries; shifting weight toward text degrades both monotonically (`0.8/0.2` drops overall recall@1 by roughly a fifth); `similarity` is inert anywhere in 0.05–0.2, so the cutoff is not the binding constraint at those levels; and `titleBoost: 3` clearly beats `1` while `6` is a wash. The intuition that a query containing an exact API name should lean keyword-heavy is simply wrong for this corpus — the sweep is the reason to believe that rather than argue about it.
...
- **Natural-language queries are the weak spot.** On the 102-query benchmark (`npm run benchmark`) bare API identifiers score 78% recall@1 pooled across corpora, while questions phrased without an identifier score 42%. Three queries miss the top 5 entirely: "how do I log in once and reuse the session" returns MCP/agent-CLI storage pages instead of the `auth` guide, "create an http server" returns `http2` results rather than `http`, and "temporal dead zone let const hoisting" collides with the `Temporal` API. The tuning sweep shows this is not fixable by rebalancing the hybrid weights — see the design note below.
